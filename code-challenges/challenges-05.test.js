...
const templateWithJQuery = () => {
  starWarsPeople.forEach(element => {
    const $section = $('#template').html();
    let div = $('<section>' + $section + '</section>');
    div.find('h2').text(element.name);
    div.find('h3').text(element.height);
    div.find('p').text(element.eye_color);
    $('main').append(div);
  });
...
const howMuchPencil = (str) => {
  ...
  for (var i = 0; i <= str.length; i++) {
    var word = str.slice(i, str.length);
    result.push(word);
  }
  return result;
};
